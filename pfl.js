...
function teamCircle(team, size = 44) {
  ...
  // On load: remove colored background so logo appears on transparent
  // On error: remove img and show colored circle with abbreviation instead
  return `
    <div class="team-circle" id="tc-${team.id}-${size}" style="width:${size}px;height:${size}px;background:${team.primary};color:${team.secondary};font-size:${fontSize}px;">
      <img src="${team.logo}" alt="${team.name} logo"
           onload="this.parentElement.style.background='transparent';this.parentElement.style.border='none';"
           onerror="this.parentElement.style.background='${team.primary}';this.parentElement.innerHTML='<span>${team.id}</span>';"
